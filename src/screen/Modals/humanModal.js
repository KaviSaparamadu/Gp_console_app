    ...
    >
      <ScrollView contentContainerStyle={styles.container}>
        <Animatable.View animation="slideInRight" duration={500}>
          ...
          {step === 1 && (
            <>
              <FormDropdown label="Country" required data={[{ label: "Select country", value: "" }, { label: "Sri Lanka", value: "sri_lanka" }, { label: "India", value: "india" }]} value={selectedCountry} onChange={setSelectedCountry} />
              ...
              <FormDropdown label="Title" required data={[{ label: "Select Title", value: "" }, { label: "Mr", value: "Mr." }, { label: "Miss", value: "Miss" }]} value={title} onChange={setTitle} />
          ...
          {step === 3 && (
            <>
              <FormDropdown label="Province" value={province} onChange={setProvince} data={[{ label: "Select Province", value: "" }, { label: "Western", value: "western" }, { label: "Central", value: "central" }]} />
              <FormDropdown label="District" value={district} onChange={setDistrict} data={[{ label: "Select District", value: "" }, { label: "Colombo", value: "colombo" }, { label: "Kandy", value: "kandy" }]} />
              <FormDropdown label="GN Division" value={gnDivision} onChange={setGnDivision} data={[{ label: "Select GN Division", value: "" }, { label: "Kolonnawa", value: "kolonnawa" }, { label: "Dehiwala", value: "dehiwala" }]} />
              ...
              {addressLines.map((line, index) => (
                <View key={index} style={styles.addressRow}>
                  <TextInput placeholder={`Address Line ${index + 1}`} style={styles.input} value={line} onChangeText={text => updateAddressLine(text, index)} />
                  {index > 0 && (
                    <TouchableOpacity onPress={() => { const updated = [...addressLines]; updated.splice(index, 1); setAddressLines(updated); }} style={{ marginLeft: 8 }}>
...
// FormInput and FormDropdown
const FormInput = ({ label, value, onChangeText, required }) => (
  <View style={{ marginBottom: 12 }}>
    <Text style={styles.label}>{required && <Text style={{ color: "black" }}>* </Text>}{label}</Text>
...
const FormDropdown = ({ label, data, value, onChange, required }) => (
  <View style={{ marginBottom: 12 }}>
    <Text style={styles.label}>{required && <Text style={{ color: "black" }}>* </Text>}{label}</Text>
    <Dropdown data={data} labelField="label" valueField="value" placeholder={`Select ${label}`} value={value} onChange={item => onChange(item.value)} style={styles.dropdown} placeholderStyle={styles.dropdownPlaceholder} selectedTextStyle={styles.dropdownSelected} itemTextStyle={styles.dropdownItem} />
  </View>
);
  ...
  <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
    {[...Array(totalSteps)].map((_, index) => (
      <View key={index} style={{ flex: 1, alignItems: "center" }}>
        <View style={{ width: 24, height: 24, borderRadius: 12, backgroundColor: index + 1 <= step ? "#f06795" : "#ddd", justifyContent: "center", alignItems: "center" }}>
          <Text style={{ color: "#fff", fontWeight: "bold", fontSize: 12 }}>{index + 1}</Text>
        </View>
        {index < totalSteps - 1 && (
          <View style={{ position: "absolute", top: 12, left: "50%", width: "100%", height: 4, backgroundColor: index + 1 < step ? "#f06795" : "#ddd", zIndex: -1 }} />
        )}
      </View>
    ))}
  </View>
...
const NavButton = ({ label, onPress, gray }) => (
  <TouchableOpacity style={{ backgroundColor: gray ? "#ccc" : "#595959", padding: 12, flex: 1, marginLeft: gray ? 0 : 5, marginRight: gray ? 5 : 0 }} onPress={onPress}>
...
const styles = StyleSheet.create({
  ...
  dropdown: { borderBottomWidth: 1, borderColor: "#ddd", height: 35, justifyContent: "center", paddingHorizontal: 4, borderRadius: 4, marginTop: 2, backgroundColor: "#fff" },
  dropdownPlaceholder: { fontSize: 13, color: "#aaa" },
  dropdownSelected: { fontSize: 13 },
  dropdownItem: { fontSize: 13 },
  ...
  addButtonText: { fontSize: 14, color: "green", fontWeight: "bold", marginLeft: 5 },
